...
def main(n, bodies=BODIES, ref='sun'):
    ...
    :param n: int: time step for the integration
    """
    system = list(bodies.values())
    pairs = combinations(system)
    # make sure sun is in the center
    offset_momentum(bodies[ref], system)
    # calculate total energy
    report_energy(system, pairs)
    # the unit of time is the mean solar day, the unit of distance is one
    # astronomical unit, and the unit of mass is the solar mass
    advance(0.01, n, system, pairs)
    # the energy should be similer with before
    report_energy(system, pairs)
